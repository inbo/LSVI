...
Data_voorwaarden <-
  read_csv2(system.file("vbdata/opname_4010_gelayout_indicatoren.csv",
                        package = "LSVI")) %>%
  ...
  left_join(data.frame(Indicator = c("dwergstruiken", "veenmoslaag",
...
test_that("ConnectieLSVIhabitats is een open RODBC-connectie", {
  expect_error(berekenLSVIbasis(ConnectieLSVIhabitats = "geenConnectie",
                                Versie = "alle",
               ...
               list(Resultaat[[1]] %>%
                      mutate_(
                        Beoordeling_criterium =
                          ~ifelse(Criterium == "Vegetatie" &
                                    (ID %in% c("Jo1380", "WT0174")),
