...
selecteerSoortenInOpname <-
  function(Data_soorten,
    ...
    assert_that(
      has_name(Data_soorten, "Soort_NL") |
        has_name(Data_soorten, "Soort_Latijn")
    )

    #een soortenlijst met alle wetenschappelijke/Nederlandse namen uit de
    #databank (zonder auteursnaam) om de schrijfwijze van de namen in de
    #lijst te controleren
    ...
    #om vergelijking te vergemakkelijken
    if (has_name(Data_soorten, "Soort_Latijn")) {
      ...
      #controleren of alle namen in de databank voorkomen, anders
      #foutmelding geven
      if (
        !all(
          !is.na(Data_soorten$WetNaamKort) &
            Data_soorten$WetNaamKort %in% Soortenlijst_Latijn
        )
      ) {
        stop("Niet alle waarden vermeld onder Data_soorten$Soort_Latijn komen overeen met wetenschappelijke namen van soorten in de databank.") #nolint
      } else {
        #testen of alle namen van de soortengroep in de opname voorkomen
        #(indien niet, ook subniveaus testen), anders een waarschuwing geven
        if (!all(Soortengroep$WetNaamKort %in% Data_soorten$WetNaamKort)) {
          ...
          if (has_name(OntbrekendeSoorten, "SoortensubgroepID") &
            ...
            if (
              !all(
                OntbrekendeSoorten$SoortensubgroepID %in%
                  Subsoortengroep$SoortensubgroepID
              )
            ) {
              warning("Niet alle te evalueren soorten zijn opgenomen onder Data_soorten$Soort_Latijn, er wordt van uitgegaan dat de niet opgenomen soorten niet waargenomen zijn") #nolint
            }
          } else {
            warning("Niet alle te evalueren soorten zijn opgenomen onder Data_soorten$Soort_Latijn, er wordt van uitgegaan dat de niet opgenomen soorten niet waargenomen zijn") #nolint
          }
      ...
      if (
        !all(
          !is.na(Data_soorten$Soort_NL) &
            Data_soorten$Soort_NL %in% Soortenlijst_NL
        )
      ) {
        stop("Niet alle waarden vermeld onder Data_soorten$Soort_NL komen overeen met Nederlandse namen van soorten in de databank.") #nolint
      } else {
        if (!all(Soortengroep$NedNaam %in% Data_soorten$Soort_NL)) {
          ...
          if (has_name(OntbrekendeSoorten, "SoortensubgroepID") &
            ...
            Soortengroep <- Soortengroep %>%
              bind_rows(Subsoortengroep)
            if (
              !all(OntbrekendeSoorten$SoortensubgroepID %in%
                   Subsoortengroep$SoortensubgroepID)
            ) {
              warning("Niet alle te evalueren soorten zijn opgenomen onder Data_soorten$Soort_NL, er wordt van uitgegaan dat de niet opgenomen soorten niet waargenomen zijn") #nolint
            }
          } else {
            warning("Niet alle te evalueren soorten zijn opgenomen onder Data_soorten$Soort_NL, er wordt van uitgegaan dat de niet opgenomen soorten niet waargenomen zijn") #nolint
          }
