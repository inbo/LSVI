...
#' @param Aggregatiemethode Keuze van de methode om tot één beoordeling per criterium of per habitatvlek of meetpunt te komen. Er zijn twee opties: (1) "RapportageHR": de beoordeling is gunstig als meer dan 50 procent van de indicatoren gunstig zijn EN als geen enkele zeer belangrijke indicator ongunstig is; (2) "1-out-all-out": de beoordeling is gunstig als alle indicatoren gunstig zijn. "1-out-all-out" is default.
#' @param na.rm Hier geef je aan hoe de berekening moet omgaan met NA waarden. Default is FALSE. Dit betekent dat NA waarden niet worden verwijderd. Hierdoor zal de indexberekening resulteren in een NA zodra één van de indicatoren NA is. Voor de berekening van de status zal dit enkel resulteren in een NA indien minstens één van de indicatoren NA is en minstens één van de indicatoren status TRUE (= gunstig) heeft. Indien na.rm = TRUE worden eventuele NA waarden verwijderd zodat status en de indices een resultaat hebben. Doordat deze dan mogelijk niet op de volledige set van indicatoren gebaseerd zijn, moet hiermee rekening gehouden worden afhankelijk van de context waarvoor de resultaten gebruikt worden.
#' @param Oppervlakte_opname Default is FALSE. Voor klassieke LSVI-opnamen op perceelsniveau laat men deze parameter best op FALSE staan. Voor gegevens van vegetatie-opnamen in proefvlakken, is het beter om de parameter op TRUE te zetten. Indien TRUE, wordt gecontroleerd of het veld Opp_m2 in Data_habitat aanwezig is en zal de LSVI berekend worden rekening houdend met de oppervlakte-afhankelijkheid van het aantal sleutelsoorten. Hierbij worden twee wijzigingen doorgevoerd ten opzichte van de default: (1) de referentiewaarden voor het aantal sleutelsoorten worden naar beneden gecorrigeerd op basis van habitatspecifieke power-law curves en de veronderstelling dat de default referentiewaarden gelden voor een oppervlakte van 5000 m2, en (2) bij de berekening van de verschilscores wordt het maximum (= +1) bereikt vanaf er meer dan of gelijk aan twee keer de referentiewaarde sleutelsoorten in de opname aanwezig zijn (in plaats van het theoretisch maximaal aantal sleutelsoorten). (1) heeft een impact op zowel de Statusberekening als op de berekening van de biotische indices. (2) heeft enkel een impact op de berekening van de biotische indices.
#'
