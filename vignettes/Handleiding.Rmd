...
Om de functies uit het package te kunnen gebruiken, moet een connectie gemaakt kunnen worden met de databank 'D0122_00_LSVIHabitatTypes' die op server 'INBO-SQL07-PRD.inbo.be' staat en alle informatie in verband met de LSVI-indicatoren bevat (zie 'Doelstellingen en inhoud package' > 'Inhoud package').  Dit betekent dat gebruikers die niet op het INBO-netwerk werken, een kopie van deze databank nodig hebben en de locatie en naam van deze databank telkens moeten specifiëren in de functie connecteerMetLSVIdb().  (Dit is een voorlopige oplossing: zodra deze databank in orde is, zal ze geïntegreerd worden in het package zelf, waardoor iedereen de functies uit het package zal kunnen gebruiken.)
...
Je steekt dus de gegevens uit je bestand 'Gegevens.csv' in de variabele 'Dataset'.  (Het laden van de library readr is enkel nodig als je dit nog niet eerder gedaan hebt.)
...
De functie berekenLSVIbasis() geeft gegevens terug in de vorm van een list van 3 tabellen.
