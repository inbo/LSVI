...
- er wordt gepoogd om deze lijst zo compatibel mogelijk te maken met INBO-databanken die gebruikt worden voor vegetatieopnamen (schrijfwijzen), de lijst is raadpleegbaar op [Zenodo](https://zenodo.org/records/10561497) en na installatie van het package met het commando `readr::read_csv2(system.file("databank/TaxonTabel.csv", package = "LSVI"))`.
...
Als dit geen resultaat geeft (bv. doordat een auteursnaam niet exact hetzelfde afgekort is of een "." ontbreekt), zal geprobeerd worden om een koppeling te maken op basis van de Gbif-tool (d.m.v. het rgbif-package):
...
Voor enkele analysevariabelen ([`bedekkingLaagPlus`](#bedekkinglaagplus), [`bedekkingLaagExcl`](#bedekkinglaagexcl) en [`aantalGroepen`](#aantalgroepen)) zijn 2 soortenlijsten nodig voor de berekening.  Dit is opgelost door een lijst te maken (= lijst van taxa met eenzelfde `TaxongroepId`) met 2 sublijsten (= lijst van taxa met eenzelfde `TaxonsubgroepId`).  Bij [`bedekkingLaagPlus`](#bedekkinglaagplus) en [`bedekkingLaagExcl`](#bedekkinglaagexcl) wordt deze lijst op basis van `TaxonsubgroepId` opgesplitst in 2 afzonderlijke lijsten, die elk in een aparte bewerking of voorbewerking gebruikt worden (zie onder [`bedekkingLaagPlus`](#bedekkinglaagplus) en [`bedekkingLaagExcl`](#bedekkinglaagexcl)).  Bij [`aantalGroepen`](#aantalgroepen) zijn de `Id`'s hernoemd om code van analysevariabele [`aantal`](#aantal) te kunnen hergebruiken:

- `TaxonsubgroepId` -> `TaxonId` zodanig dat de originele `TaxongroepId`'s aanzien worden als de items (soorten) die geteld moeten worden (terwijl bedekkingen van taxa binnen eenzelfde `TaxonId` met de formule van Fisher geaggregeerd worden)
