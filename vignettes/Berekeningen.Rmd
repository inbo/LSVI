...
## AnalyseVariabele {#analysevariabele} <!-- spell-check: ignore -->

Bij het berekenen van een `AnalyseVariabele` wordt vaak de (interne) functie `selecteerKenmerkenInOpname()` aangeroepen om de soorten of kenmerken uit de opname van de gebruiker te selecteren.  Om de workflow van deze functie niet te herhalen in elk van onderstaande analysevariabelen (berekeningswijzen), bespreken we hem hier apart.
...
## bedekkingExcl <!-- spell-check: ignore -->

Bij de analysevariabele `bedekkingExcl` gaat de voorwaarde over de totale bedekking van de soorten of kenmerken die niet in een lijst voorkomen, bv. alle soorten behalve de sleutelsoorten.

Bij voorwaarden met deze analysevariabele worden de soorten of kenmerken geselecteerd op basis van een functie `deselecteerKenmerkenInOpname()`, die in de opname van de gebruiker de soorten of kenmerken schrapt die in resp. de taxonlijst of studielijst voorkomen (en de overblijvende soorten of kenmerken vormen de selectie).
Als een taxonlijst én studielijst opgegeven is, berekent de functie de totale bedekking van soorten die niet in de opgegeven taxonlijst staan en wél in de opgegeven vegetatielaag voorkomen (bv. alle soorten uit de kruidlaag behalve de soorten uit de taxonlijst).
Verder gebeurt de berekening analoog aan deze van de analysevariabele [`bedekking`](#bedekking): met de formule van Fisher wordt de totale bedekking van deze selectie berekend.

## bedekkingLaag {#bedekkinglaag} <!-- spell-check: ignore -->
...
## bedekkingLaagExcl {#bedekkinglaagexcl} <!-- spell-check: ignore -->

De analysevariabele `bedekkingLaagExcl` is zeer analoog aan [`bedekkingLaag`](#bedekkinglaag), maar lost een taxonlijst-gerelateerd probleem op dat voorkomt bij de taxonlijst van de boomlaag.  Om ervoor te zorgen dat ook in de opname van de gebruiker ingevoerde genera als boom beschouwd worden, bestaat de lijst van bomen en struiken in de databank uit genera.  Lagere taxonomische niveaus worden automatisch ook meegenomen omdat hier volgens rank "genus" geselecteerd wordt (zie [Afhandeling taxonlijsten](#taxonlijsten)).  Er is echter 1 moeilijkheid: _Salix repens_ zou niet meegenomen mogen worden want dit is geen boom (die wel in de struiklaag zou kunnen voorkomen).  Anderzijds zijn er een aantal moeilijk te determineren _Salix_-soorten die evt. als genus in een opname zou kunnen voorkomen, maar die allen bomen zijn.  We gaan ervan uit dat _Salix repens_ wel herkend wordt en dus tot op soortniveau in een opname toegevoegd zal worden, en enkel moeilijk te herkennen _Salix_-soorten als genus toegevoegd worden.  Vanuit die redenering worden "_Salix_ sp." en de _Salix_-soorten als boom beschouwd, met uitzondering van _Salix repens_.

Dit probleempje heeft aanleiding gegeven voor een nieuwe analysevariabele, die uiteraard ook ruimer gebruikt kan worden.  Bij voorwaarden met deze analysevariabele zijn er 2 `TaxonGroepCode`'s verwijzend naar 2 taxonlijsten: een lange lijst (bv. alle genera van bomen) en een kort lijstje (bv. _Salix repens_).

Bij de berekening zal eerst gecheckt worden of er inderdaad 2 taxonlijsten zijn, en in dat geval zullen eerst de taxa van het kortste lijstje geschrapt worden uit de opname van de gebruiker, waarna de voorwaarde uitgerekend wordt met de lange taxonlijst volgens analysevariabele [`bedekkingLaag`](#bedekkinglaag).

## bedekkingLaagPlus {#bedekkinglaagplus} <!-- spell-check: ignore -->

Bij de analysevariabele `bedekkingLaagPlus` gaat de voorwaarde over de totale bedekking van een vegetatielaag en een taxongroep, bv. de bedekking van de moslaag en klimop.  Verschillend van [`bedekkingLaag`](#bedekkinglaag) zijn er 2 `TaxonGroepCode`'s verwijzend naar 2 taxonlijsten (zie [Afhandeling taxonlijsten](#taxonlijsten)): een lange lijst met alle soorten van de vegetatielaag die in studielijst vermeld is (in het voorbeeld alle mossen), en een korte lijst met de soorten waarvan de bedekking hierbij opgeteld moet worden (in het voorbeeld klimop).

Bij de berekening zal eerst gecheckt worden of er inderdaad 2 taxonlijsten zijn.  Op basis van de kortste lijst wordt de bedekking van deze soort(en) berekend volgens de analysevariabele [`bedekking`](#bedekking) (waarbij de studielijst buiten beschouwing gelaten wordt).  Op basis van het langste lijst en de studielijst wordt de bedekking van de vegetatielaag berekend volgens de analysevariabele [`bedekkingLaag`](#bedekkinglaag).  Daarna wordt de voorwaarde berekend door beide bedekkingen op te tellen volgens de formule van Fisher (zie onder [`bedekking`](#bedekking)).  (Ingeval er geen 2 taxonlijsten zijn, zal de voorwaarde als weergegeven in de databank, berekend worden volgens analysevariabele [`bedekkingLaag`](#bedekkinglaag)).

## bedekkingSom <!-- spell-check: ignore -->
...
De bedekkingen worden berekend volgens de analysevariabele [`bedekking`](#bedekking) voor enerzijds de studielijst (waarbij `TaxonGroepCode` buiten beschouwing gelaten wordt), en anderzijds voor `TaxonGroepCode` (waarbij de studielijst buiten beschouwing gelaten wordt).  Daarna wordt de voorwaarde berekend door beide bedekkingen op te tellen.  Vermits de studielijst doorgaans geen vegetatie bevat, wordt niet uitgegaan van een gedeeltelijke overlap en wordt een gewone som gebruikt.

## maxBedekking {#maxbedekking} <!-- spell-check: ignore -->
...
## maxBedekking2s <!-- spell-check: ignore -->

De analysevariabele `maxBedekking2s` is gelijkaardig aan [`maxBedekking`](#maxbedekking) met als enige verschil dat bij 2 of meer aanwezige soorten of kenmerken, de bedekkingen van de 2 soorten of kenmerken met de hoogste bedekking opgeteld worden volgens de formule van Fisher (zie onder [`bedekking`](#bedekking)).

## maxBedekkingExcl <!-- spell-check: ignore -->

Bij de analysevariabele `maxBedekkingExcl` gaat de voorwaarde over de maximale bedekking van de soorten of kenmerken die niet in een lijst voorkomen, bv. alle soorten behalve de sleutelsoorten.
...
## aandeelKruidlaag <!-- spell-check: ignore -->

De analysevariabele `aandeelKruidlaag` heeft eigenlijk een ietwat slecht gekozen naam, omdat de functie in principe het aandeel ten opzichte van eender welke vegetatielaag berekent.  Bij voorwaarden met deze analysevariabele verwijst `TaxonGroepCode` naar een lijst met de relevante soorten om het aandeel te berekenen, en studielijst vermeldt de vegetatielaag waarin dit aandeel bestudeerd wordt.

De totale bedekking van de relevante soorten wordt berekend door de voorwaarde (met `TaxonGroepCode` en studielijst) te berekenen volgens de analysevariabele [`bedekking`](#bedekking).  Voor de totale bedekking van de vegetatielaag zal eerst gecheckt worden of het kenmerk uit de studielijst (de vegetatielaag) voorkomt in de opname van de gebruiker.
...
## aantalGroepen {#aantalgroepen} <!-- spell-check: ignore -->

De analysevariabele `aantalGroepen` gaat specifiek over kenmerken die eigenlijk soortenlijsten zijn, bv. helofyten.  Deze kenmerken zijn in de databank enerzijds weergegeven in de studielijst, en anderzijds vermeldt `TaxonGroepCode` meerdere taxonlijsten die elk overeenkomen met een van de kenmerken (en die de relevante soorten bevatten).

Bij voorwaarden met deze analysevariabele zal eerst gecheckt worden of kenmerken uit de studielijst voorkomen in de opname van de gebruiker.

- Als dat zo is, dan zal het aantal afgeleid worden van de kenmerken in de studielijst.  (Meer concreet: de berekening gebeurt zoals bij analysevariabele [`aantal`](#aantal) nadat eerst de taxongroepen uit de voorwaarden geschrapt zijn.)
- Als dat niet zo is, dan zal het aantal afgeleid worden op basis van opgegeven taxonlijsten.  Concreet zal in de originele voorwaarde de studielijst geschrapt worden, waarna voor elk van de taxongroepen de aanwezigheid van taxa afgeleid wordt door de analysevariabele [`aantal`](#aantal) te berekenen.  Uiteindelijk wordt het aantal taxongroepen geteld waar het berekende aantal taxa groter is dan 0.  (Hoewel we enkel aanwezigheid nodig hebben, herbruiken we hier de berekening van analysevariabele [`aantal`](#aantal) om geen gelijkaardige code te moeten ontwikkelen en onderhouden.  Zie [Afhandeling taxonlijsten](#taxonlijsten) voor meer details over de berekeningen.)

## scoresom <!-- spell-check: ignore -->
...
De analysevariabele `meting` (of `meting_xxx`) staat in de databank bij voorwaarden waarvoor geen specifieke berekeningswijze ontwikkeld is.  Voorlopig is de bedoeling dat voor deze voorwaarden een waarde ingevoerd wordt via `Data_voorwaarden`; de berekening van deze analysevariabele geeft een foutmelding als dit niet het geval is.  Idee is om op termijn op zoek te gaan naar een oplossing om ingevoerde kenmerken in de mate van het mogelijke toch te koppelen aan voorwaarden van deze analysevariabele.

# Afhandeling taxonlijsten {#taxonlijsten}

## Gebruikte taxonlijsten in package

Idealiter worden de door de gebruiker ingevoerde taxonnamen herkend en correct behandeld bij de berekening.  Om dit zo goed mogelijk op te vangen, bevat het package een __generieke taxonlijst__ met:

- taxa van meerdere taxonomische niveaus (genera, soorten, ondersoorten,...)
- zowel "accepted names" als "junior synonyms"
- Latijnse en Nederlandse namen
- voor elk van deze records de taxa van hogere niveaus waartoe ze horen (genus, familie, orde,..) en overeenkomstige Gbif-keys voor alle niveaus, dus een unieke code voor elk taxon (en ingeval van "junior synonyms" ook de "accepted name")
- er wordt gepoogd om deze lijst zo compatibel mogelijk te maken met INBO-databanken die gebruikt worden voor vegetatieopnamen (schrijfwijzen), de lijst is raadpleegbaar op [Zenodo](https://zenodo.org/records/10561497).
- de taxonomie en voornamelijk synonymy van deze tabel is afgeleid van [Florabank](https://flora.inbo.be), waarmee de taxonomische inzichten van [worldfloraonline.org](https://worldfloraonline.org/) en de Belgische flora gevolgd worden (die soms afwijken van Gbif)

De indicatoren en voorwaarden in de __databank__ van het package zijn gekoppeld aan __taxonlijsten__ met enkel "accepted names" met voor elk taxon:

- de Latijnse en Nederlandse naam (voor uitvoer in functies als `geefSoortenlijst()` en `maakLSVIrapport()`)
- de overeenkomstige Gbif-key
- de rank van het taxon

## Herkenning van ingevoerde namen {#herkenning_namen}

De "herkenning" van de door een gebruiker ingevoerde namen gebeurt in een eerste stap door een exacte koppeling te maken met de generieke taxonlijst in het package.
Als dit geen resultaat geeft (bv. doordat een auteursnaam niet exact hetzelfde afgekort is of een "." ontbreekt), zal geprobeerd worden om een koppeling te maken op basis van de [Gbif-tool](https://www.gbif.org/tool/81747/rgbif) (d.m.v. het rgbif-package):

- voor Latijnse namen met commando `rgbif::name_backbone(...)`
- voor Nederlandse namen met commando `rgbif::name_lookup(...)$data`
- voor (in de taxonlijst ontbrekende) Gbif-keys met commando `name_usage(...)$data`

Als deze koppeling gemaakt kan worden, zal geprobeerd worden om toch zoveel mogelijk info uit de generieke taxonlijst in het package (met de taxonomische inzichten uit Florabank) te gebruiken:

- in de eerste plaats wordt geprobeerd om de gevonden Gbif-key te koppelen met de taxonlijst uit het package om de key van de accepted name en de hogere taxonniveaus over te nemen
- als dit niet lukt, zal ook de key van de accepted name via Gbif opgezocht worden en zal geprobeerd worden om deze te koppelen met de taxonlijst uit het package om de hogere taxonniveaus over te nemen
- als dit ook niet lukt, zullen de door Gbif voorgestelde hogere taxonniveaus overgenomen worden

Als er geen koppeling gemaakt kan worden, zal dit een error geven zodat de gebruiker die naam kan aanpassen vooraleer de berekeningen gebeuren (om te vermijden dat de lange berekeningen onnodig gebeuren).

Finaal zal de functie `berekenLSVIbasis()` behalve de resultaten ook een tabel `Soortenlijst` teruggeven waaruit afgeleid kan worden hoe de ingevoerde taxa geïnterpreteerd zijn door het package.
Als de functie Gbif raadpleegt omdat minstens één door de gebruiker ingevoerde naam niet in de taxonlijst voorkomt, dan geeft ze een warning om deze tabel na te kijken.
Deze waarschuwing komt er enerzijds omdat Gbif bij z'n vertaling geen rekening houdt met auteursnamen of hiermee gerelateerde tussenvoegsels zoals "auct.", "auct. non", "pro parte",..., wat tot een fout taxon kan leiden, en anderzijds omdat de synonymy in Gbif soms afwijkt van die van Florabank die doorgaans in het Nederlandse taalgebied gehanteerd wordt.
Het is dus belangrijk om dit na te kijken en indien nodig de ingevoerde naam aan te passen zodat deze wel correct herkend wordt.
Te verwachten fouten bij vertaling door Gbif zijn dat het laatste woord (naam van soort of subsoort of ...) genegeerd wordt als een auteursnaam ontbreekt (omdat Gbif het beschouwt als auteursnaam), of dat een taxon breder of enger geïnterpreteerd kan worden doordat dezelfde naam door meerdere auteurs is beschreven met een engere of bredere betekenis (bv. voor en na opsplitsing in meerdere taxa).

Fouten in de taxonlijst in het package mogen altijd gemeld worden via [een issue](https://github.com/inbo/LSVI/issues) (of aan de beheerder van dit package).

## Ingevoerde namen selecteren voor "berekeningen"

Bij het selecteren van de voor de voorwaarde relevante namen uit de opname op basis van een taxonlijst in de databank (met `selecteerKenmerkenInOpname()`, zie onder [`AnalyseVariabele`](#analysevariabele)), worden de taxa in de databank gekoppeld met de tabel `Soortenlijst` met de door gebruiker ingevoerde taxa (zie [Herkenning van ingevoerde namen](#herkenning_namen) hierboven).
Dit gebeurt door de rank en Gbif-key uit de databank te koppelen aan de records van tabel `Soortenlijst` die in de kolom van de desbetreffende rank een overeenkomstige Gbif-key hebben.
Als meerdere taxa van een lager niveau voorkomen die dezelfde opgegeven rank hebben (bv. meerdere soorten van hetzelfde genus), dan worden deze bedekkingen geaggregeerd door middel van de formule van Fisher (zie onder [bedekking](#bedekking)).
Daarna gebeurt op de overgebleven taxa de uiteindelijke bewerking als beschreven bij de analysevariabelen (aantallen tellen, bedekkingen aggregeren,...).

Voor enkele analysevariabelen ([`bedekkingLaagPlus`](#bedekkinglaagplus), [`bedekkingLaagExcl`](#bedekkinglaagexcl) en [`aantalGroepen`](#aantalgroepen)) zijn 2 of meer soortenlijsten nodig voor de berekening.  Dit is opgelost door meerdere taxonlijsten (= lijst van taxa met eenzelfde `TaxonGroepCode`) aan eenzelfde voorwaarde te hangen.  Bij [`bedekkingLaagPlus`](#bedekkinglaagplus) en [`bedekkingLaagExcl`](#bedekkinglaagexcl) wordt deze lijst op basis van `TaxonGroepCode` beschouwd als 2 afzonderlijke lijsten, die elk in een aparte bewerking of voorbewerking gebruikt worden (zie onder [`bedekkingLaagPlus`](#bedekkinglaagplus) en [`bedekkingLaagExcl`](#bedekkinglaagexcl)).  Bij [`aantalGroepen`](#aantalgroepen) wordt de code van analysevariabele [`aantal`](#aantal) toegepast op elk van de taxongroepen om daarna af te leiden van hoeveel groepen er taxa aanwezig zijn.

Ook bij andere analysevariabelen kan een voorwaarde meerdere taxonlijsten hebben, meestal als gevolg van het feit dat deze sublijsten bij andere voorwaarden als aparte lijst gebruikt worden.  In deze gevallen is het voorkomen van meerdere taxonlijsten een louter praktisch gegeven en heeft het geen invloed op de berekening.
...
- theoretisch maximum is weergegeven in de databank en wordt bepaald als volgt:
...
Voor de aggregatie naar de volledige opname worden dezelfde 2 aggregatiemethoden opnieuw gebruikt uitgaande van de geaggregeerde resultaten op het niveau van een criterium.  
